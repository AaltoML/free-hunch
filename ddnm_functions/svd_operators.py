...
class A_functions:
    ...
    def A_with_zeros(self, vec):
        """
        Returns a vector of the same dimension as vec, where:
        - Elements corresponding to kept dimensions contain A(vec)
        - Elements corresponding to dropped dimensions are zero
        """
        # Get the A operation result
        a_result = self.A(vec)        
        return self.V(self.add_zeros(a_result))
...
def create_inpainting_operator(channels, img_dim, mask_opt, device):
    """
    Creates an Inpainting operator that matches the masking behavior of InpaintingOperator.
    
    Args:
        channels (int): Number of image channels (typically 3 for RGB)
        img_dim (int): Size of image (assumes square images)
        mask_opt (dict): Mask configuration with keys:
            - mask_type: 'box', 'random', 'both', or 'extreme'
            - mask_len_range: (min, max) tuple for box size
            - mask_prob_range: (min, max) tuple for random mask probability
            - image_size: Size of image
            - margin: Margin size for box masks
        device: PyTorch device
    
    Returns:
        Inpainting: An inpainting operator configured with the specified mask
    """
    
    # Create mask using MaskGenerator
    mask_generator = MaskGenerator(**mask_opt)
    dummy_img = torch.randn(1, channels, img_dim, img_dim).to(device)
    mask = mask_generator(dummy_img)[0]  # Take first batch
        
    # Convert mask to indices
    # mask is 1 for kept pixels, 0 for masked pixels
    # Flatten across all channels since Inpainting operator works with flattened images
    mask_flat = mask.view(-1)
    missing_indices = torch.where(mask_flat == 0)[0]
    
    # Create and return Inpainting operator
    return Inpainting(channels, img_dim, missing_indices, device)
...
class MaskGenerator:
    def __init__(self, mask_type, mask_len_range=None, mask_prob_range=None,
        ...
        """
        Args:
            mask_type (str): Type of mask to generate ('box', 'random', 'both', 'extreme')
            mask_len_range (tuple): (min, max) range for box size
            mask_prob_range (tuple): (min, max) range for random mask probability
            image_size (int): Size of the image (assumes square)
            margin (tuple): Margins for box mask placement
        """
    ...
    def _random_sq_bbox(self, img, mask_shape, image_size=256, margin=(16, 16)):
        ...
        # Use center position instead of random
...
class Deblurring(A_functions):
    ...
    def __init__(self, kernel, channels, img_dim, device, ZERO = 3e-2, use_ddnm_kernel_params=False):
        self.img_dim = img_dim
        self.channels = channels
                
        kernel = kernel[kernel.shape[0]//2,:] / kernel[kernel.shape[0]//2,:].sum()
        
        if use_ddnm_kernel_params:
            sigma = 10
            pdf = lambda x: torch.exp(torch.Tensor([-0.5 * (x / sigma) ** 2]))
            kernel = torch.Tensor([pdf(-2), pdf(-1), pdf(0), pdf(1), pdf(2)]).to(kernel.device)
        ...
        # # Assuming kernel is 2D with shape (k, k)
        # k_h, k_w = kernel.shape
        # pad_h, pad_w = k_h // 2, k_w // 2
        
        # # Build 2D conv matrix
        # A_small = torch.zeros(img_dim, img_dim, device=device)
        # for i in range(img_dim):
        #     for j in range(img_dim):
        #         # Calculate the valid kernel positions
        #         for ki in range(k_h):
        #             for kj in range(k_w):
        ...
        #                 if 0 <= si < img_dim and 0 <= sj < img_dim:
        #                     A_small[i, j] += kernel[ki, kj].item()
